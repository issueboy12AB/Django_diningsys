...
class Orders(models.Model):
    ...
    status = models.IntegerField(default=1)  # 1:进行中 2:无效 3:已完成 9:已删除
    payment_status = models.IntegerField(default=1)  # 1:未支付 2:已支付 3:已退款
    create_at = models.DateTimeField(default=datetime.now)  # 创建时间
    update_at = models.DateTimeField(default=datetime.now)  # 修改时间

    def toDict(self):
        return {'id': self.id, 'shop_id': self.shop_id, 'user_id': self.user_id, 'member_id': self.member_id,
                'money': self.money, 'status': self.status, 'payment_status': self.payment_status,
                'create_at': self.create_at.strftime('%Y-%m-%d %H:%M:%S'),
                'update_at': self.update_at.strftime('%Y-%m-%d %H:%M:%S')}
